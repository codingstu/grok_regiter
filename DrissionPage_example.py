...
def get_faker_locale_by_ip():
    import requests
    try:
        r = requests.get("http://ip-api.com/json/", timeout=5)
        country_code = r.json().get("countryCode", "US")
    except Exception:
        country_code = "US"
        
    locale_map = {
        "CN": "zh_CN", "US": "en_US", "GB": "en_GB",
        "JP": "ja_JP", "KR": "ko_KR", "FR": "fr_FR",
        "DE": "de_DE", "RU": "ru_RU", "TW": "zh_TW",
        "HK": "zh_HK", "SG": "zh_CN", "IN": "en_IN",
        "AU": "en_AU", "CA": "en_CA"
    }
    return locale_map.get(country_code, "en_US")

def build_profile():
    # 生成一组可重复使用的注册资料，密码至少包含大小写、数字和特殊字符。
    try:
        from faker import Faker
        locale = get_faker_locale_by_ip()
        fake = Faker(locale)
        given_name = fake.first_name()
        family_name = fake.last_name()
        print(f"[*] 根据IP所在地 ({locale}) 生成随机人名: {given_name} {family_name}")
    except ImportError:
        given_name = "Fan"
        family_name = "YuXi"
